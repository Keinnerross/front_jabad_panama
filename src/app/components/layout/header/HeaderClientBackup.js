...
export const HeaderClient = ({ data, colorTheme, customPagesData, customEventsData }) => {
    ...
    const menuItems = [
        ...
        // 3. Events dropdown (only if there are custom events available)
        ...(customEventsData && customEventsData.length > 0 ? [{
            name: "Events",
            hasDropdown: true,
            subItems: customEventsData.map(event => ({
                name: event.name,
